...
export class PreAwardApiService {
    ...
    constructor(server: HapiServer) {
        ...
        // Determine the Pre-Award API base URL from environment or config
        const apiHost = process.env.API_HOST || config.API_HOST || 'localhost:3002';
        this.apiBaseUrl = process.env.PRE_AWARD_API_URL || `http://${apiHost}/forms`;
    ...
    /**
     * Fetches the published JSON for a specific form.
     ...
     */
    async getPublishedForm(name: string, request?: HapiRequest): Promise<any> {
        ...
        try {
            ...
            // The API returns the published_json object directly
            return payload;
    ...
    async getFormHash(name: string, request?: HapiRequest): Promise<string | null> {
        ...
        try {
            ...
            const data = payload as FormHash;

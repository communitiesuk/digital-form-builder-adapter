...
When("I submit the mhclg form with the button title {string}", (title) => {
  cy.findByRole("button", { name: title }).click();
});

When("I create a component for new UI", (table) => {
  const {
    page,
    component,
    title,
...
When("I open {string} in new UI", (button) => {
...
When("I open the link {string} for new UI", (link) => {
...
When(
  "I select {string} for the field with the name {string} in new UI",
  (option, fieldName) => {
    cy.get(`[name="${fieldName}"]`).select(option);
  }
);

When("I add the condition in new UI", () => {
  cy.findByRole("button", { name: "Add" }).click();
});

When("I save the condition in new UI", () => {
  cy.findByRole("link", { name: "Save" }).click();
});

Then("I see the condition {string} in new UI", (string) => {
  cy.findByText(string, { exact: false });
});

When("I close the flyout in new UI", () => {
  // for whatever reason cypress and/or testing library can't find this a href.
  cy.reload();
});

When("I select the page link with test id {string} in new UI", (string) => {
...
When("I select the condition {string} in new UI", (string) => {
  cy.findByLabelText("Select a condition").select(string);
});

When("I save my link in new UI", () => {
  cy.findByRole("button", { name: "Save" }).click();
});
